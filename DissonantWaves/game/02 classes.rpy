init python:
    ...
    class Person:
        def __init__(self, character, name, alive = True, affection=0, tagline="", preferedMc=[], images=None):
            self.c = character
            self.n = name
            self.alive = alive
            self.affection = affection
            self.tagline = tagline
            self.preferedMc = preferedMc
            self.i = images
        ...
        def returnAffectionState(self, player):
            ...
            preferred_values = [trait_map[trait] for trait in self. preferedMc if trait in trait_map]
